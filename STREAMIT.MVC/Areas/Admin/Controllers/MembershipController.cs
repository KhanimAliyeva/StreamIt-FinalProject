...
namespace STREAMIT.MVC.Areas.Admin.Controllers
{
    ...
    public class MembershipController : Controller
    {
        ...
        {
            var memberships = await SafeGetListFromApi<GetMembershipDto>("api/Membership");

            int pageSize = 5;
            int totalCount = memberships.Count;
            ...
            var model = new PagedMembershipViewModel
            {
        ...
        public async Task<IActionResult> Create(CreateMembershipDto dto)
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("api/Membership", dto);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                ModelState.AddModelError(string.Empty, $"API Error {(int)response.StatusCode}: {content}");
                return View(dto);
            }
        ...
        {
            var membership = await SafeGetFromApi<GetMembershipDto>($"api/Membership/{id}");
            ...
            var dto = new UpdateMembershipDto
            {
                ...
                VideoQuality = membership.VideoQuality,
                MaxDevices = membership.MaxDevices,
                HasAds = membership.HasAds,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(UpdateMembershipDto dto)
        {
            if (!ModelState.IsValid) return View(dto);

            var response = await _httpClient.PutAsJsonAsync($"api/Membership/{dto.Id}", dto);
            var content = await response.Content.ReadAsStringAsync();
        ...
        {
            var response = await _httpClient.DeleteAsync($"api/Membership/{id}");
